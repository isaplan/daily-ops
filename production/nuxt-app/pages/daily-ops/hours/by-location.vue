  ...
  <div class="space-y-6">
    ...
    <UCard>
      ...
      <div class="grid gap-4 md:grid-cols-4">
        ...
        <div class="space-y-2"><label class="text-sm font-medium">Sort By</label><USelectMenu v-model="filters.sortBy" :items="[{ label: 'Location Name', value: 'location_name' }, { label: 'Total Hours', value: 'total_hours' }]" value-attribute="value" class="w-full" @update:model-value="fetchHours" /></div>
      </div>
    ...
    <div v-if="hoursData.length > 0" class="grid gap-4 md:grid-cols-3">
      ...
      <UCard><template #header><span class="text-sm font-medium">Total Locations</span></template><p class="text-2xl font-bold">{{ hoursData.length }}</p></UCard>
    </div>
    <UCard>
      <template #header><h2 class="font-semibold">Hours by Location</h2><p class="text-sm text-gray-500">{{ loading ? 'Loading...' : `${hoursData.length} location(s) found` }}</p></template>
