<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-3xl font-bold text-gray-900">Sales by Table (V2)</h1>
      <p class="text-gray-500">Register business day + hour from <code class="text-xs">bork_sales_by_table</code></p>
    </div>
    ...
    <UCard>
      <template #header>
        <h2 class="font-semibold">Tables ({{ paginationTotal }} total · {{ data.length }} on this page)</h2>
      </template>
      ...
      <template v-else>
        <div class="overflow-x-auto">
          <table class="w-full text-left text-sm">
            ...
            <tbody
              v-for="(row, i) in data"
              :key="`tbl-v2-${String(row.business_date)}-${row.business_hour}-${String(row.tableNumber)}-${i}`"
            >
              <tr class="border-b transition-colors hover:bg-gray-50 cursor-pointer" @click="toggleExpanded(i)">
                ...
                <td class="py-2 pr-4">{{ row.business_hour }} → {{ formatRegisterHourLabel(Number(row.business_hour ?? 0)) }}</td>
                <td class="py-2 pr-4">{{ row.tableNumber || '—' }}</td>
              ...
              <tr v-if="expandedRows.includes(i)" class="border-b bg-gray-50">
                <td colspan="7" class="py-4 px-4">
                  <div class="ml-4">
                    <div v-if="productList(row).length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-2">
                      <div
                        v-for="(prod, idx) in productList(row)"
                        :key="idx"
                        class="bg-white p-2 rounded border border-gray-200 text-xs"
                      >
                        <div class="font-semibold">{{ prod.productName }}</div>
                        <div class="text-gray-600">€{{ Number(prod.revenue).toFixed(2) }} · Qty {{ prod.quantity }}</div>
                      </div>
                    </div>
                    <p v-else class="text-sm text-gray-500">No products.</p>
                  </div>
                </td>
              </tr>
            </tbody>
