<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-3xl font-bold text-gray-900">Sales by Day</h1>
      <p class="text-gray-500">View total sales revenue and quantity per day across all locations</p>
    </div>
    ...
    <UCard>
      <template #header>
        ...
        <p class="text-sm text-gray-500">Filter and sort sales by day</p>
      </template>
    ...
    <UCard>
      <template #header>
        <h2 class="font-semibold">Sales by Day</h2>
        <p class="text-sm text-gray-500">
          {{ loading ? 'Loading...' : `${salesData.length} day(s) found` }}
        </p>
      </template>
...
const totalRecords = computed(() => {
  return salesData.value.reduce((sum: number, row: Record<string, unknown>) => sum + (Number(row.record_count ?? 0)), 0)
})
