<template>
  <div class="space-y-6">
    ...
    <UCard>
      <template #header>
        <h2 class="font-semibold">Workers ({{ paginationTotal }} total · {{ data.length }} on this page)</h2>
      </template>
      ...
      <template v-else>
        <div class="overflow-x-auto">
          <table class="w-full text-left text-sm">
            ...
            <tbody
              v-for="(row, i) in data"
              :key="`wrk-v2-${String(row.business_date)}-${row.business_hour}-${String(row.workerId)}-${i}`"
            >
