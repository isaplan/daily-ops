<template>
  <div class="space-y-6">
    <div>
      ...
      <p class="text-gray-500">View total sales revenue and quantity per product</p>
    </div>
    ...
    <UCard>
      <template #header>
        ...
        <p class="text-sm text-gray-500">Filter and sort sales by product</p>
      </template>
    ...
    <UCard>
      <template #header>
        <h2 class="font-semibold">Sales by Product</h2>
        <p class="text-sm text-gray-500">
          {{ loading ? 'Loading...' : `${salesData.length} product(s) found` }}
        </p>
      </template>
          ...
          <table class="w-full text-left text-sm">
            ...
            <tbody>
              <tr v-for="(row, i) in salesData" :key="i" class="border-b transition-colors hover:bg-gray-50">
                <td class="py-2 pr-4">{{ row.product_name || 'Unknown' }}</td>
