<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-3xl font-bold text-gray-900">Sales by Location</h1>
      <p class="text-gray-500">View total sales revenue and quantity per location</p>
    </div>
    ...
    <UCard>
      <template #header>
        ...
        <p class="text-sm text-gray-500">Filter and sort sales by location</p>
      </template>
    ...
    <div v-if="salesData.length > 0" class="grid gap-4 md:grid-cols-3">
      ...
      <UCard>
        <template #header>
          <span class="text-sm font-medium">Total Locations</span>
        </template>
    ...
    <UCard>
      <template #header>
        <h2 class="font-semibold">Sales by Location</h2>
        <p class="text-sm text-gray-500">
          {{ loading ? 'Loading...' : `${salesData.length} location(s) found` }}
        </p>
      </template>
          ...
          <table class="w-full text-left text-sm">
            <thead>
              <tr class="border-b">
                ...
                <th class="pb-2 pr-4 font-medium">Products</th>
            ...
            <tbody>
              <tr v-for="(row, i) in salesData" :key="i" class="border-b transition-colors hover:bg-gray-50">
                ...
                <td class="py-2 pr-4">{{ row.product_count ?? 0 }}</td>
...
const totalRevenue = computed(() => {
  return salesData.value.reduce((sum: number, row: Record<string, unknown>) => sum + Number(row.total_revenue ?? 0), 0)
})

const totalQuantity = computed(() => {
  return salesData.value.reduce((sum: number, row: Record<string, unknown>) => sum + Number(row.total_quantity ?? 0), 0)
})
