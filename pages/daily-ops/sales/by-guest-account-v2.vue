<template>
  <div class="space-y-6">
    ...
    <UCard>
      <template #header>
        <h2 class="font-semibold">Guest accounts ({{ paginationTotal }} total · {{ data.length }} on this page)</h2>
      </template>
      ...
      <template v-else>
        <div class="overflow-x-auto">
          <table class="w-full text-left text-sm">
            ...
            <tbody
              v-for="(row, i) in data"
              :key="`gst-v2-${String(row.business_date)}-${row.business_hour}-${String(row.accountName)}-${i}`"
            >
              <tr class="border-b transition-colors hover:bg-gray-50 cursor-pointer" @click="toggleExpanded(i)">
                ...
                <td class="py-2 pr-4">{{ row.accountName || '—' }}</td>
              ...
              <tr v-if="expandedRows.includes(i)" class="border-b bg-gray-50">
                <td colspan="7" class="py-4 px-4">
                  <div class="ml-4 text-xs text-gray-600 mb-2">Worker: {{ row.workerName || '—' }}</div>
