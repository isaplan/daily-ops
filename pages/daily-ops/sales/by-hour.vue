<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-3xl font-bold text-gray-900">Sales by Hour</h1>
      <p class="text-gray-500">Hourly sales aggregation from Bork API</p>
    </div>
      ...
      <template v-else>
        <div class="overflow-x-auto">
          <table class="w-full text-left text-sm">
            ...
            <tbody
              v-for="(row, i) in data"
              :key="`by-hour-${String(row.date)}-${row.hour}-${String(row.locationId)}-${i}`"
            >
                ...
                <td colspan="7" class="py-4 px-4">
                  <div class="ml-4 space-y-4">
                    ...
                    <div class="pt-3 border-t border-gray-200">
                      ...
                      <p v-if="detailByKey[rowKey('hour', row)]?.loading" class="text-sm text-gray-500">Loading products…</p>
                      <p v-else-if="detailByKey[rowKey('hour', row)]?.error" class="text-sm text-red-600">
                        {{ detailByKey[rowKey('hour', row)]?.error }}
                      </p>
                      <div v-else-if="(detailByKey[rowKey('hour', row)]?.items ?? []).length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-2">
                        <div
                          v-for="(prod, idx) in detailByKey[rowKey('hour', row)]?.items ?? []"
                          :key="idx"
...
/**
 * @registry-id: dailyOpsSalesByHour
...
const toggleExpanded = (index: number) => {
  ...
  if (idx > -1) {
    ...
    if (row) void ensureProducts('hour', row)
  }
}
