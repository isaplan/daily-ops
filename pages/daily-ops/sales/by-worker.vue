<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-3xl font-bold text-gray-900">Sales by Worker</h1>
      <p class="text-gray-500">Per-worker sales aggregation from Bork API</p>
    </div>
          ...
          <table class="w-full text-left text-sm">
            <thead>
              <tr class="border-b">
                ...
                <th class="pb-2 pr-4 font-medium">Hour</th>
            ...
            <tbody
              v-for="(row, i) in data"
              :key="`by-worker-${String(row.date)}-${row.hour}-${String(row.locationId)}-${String(row.workerId)}-${i}`"
            >
              <tr class="border-b transition-colors hover:bg-gray-50 cursor-pointer" @click="toggleExpanded(i)">
                ...
                <td class="py-2 pr-4">{{ String(row.hour).padStart(2, '0') }}:00</td>
                <td class="py-2 pr-4">{{ row.workerName || row.workerId || 'Unknown' }}</td>
                ...
                <td colspan="8" class="py-4 px-4">
                  <div class="ml-4 space-y-4">
                    ...
                    <div class="pt-3 border-t border-gray-200">
                      ...
                      <p v-if="detailByKey[rowKey('worker', row)]?.loading" class="text-sm text-gray-500">Loading products…</p>
                      <p v-else-if="detailByKey[rowKey('worker', row)]?.error" class="text-sm text-red-600">
                        {{ detailByKey[rowKey('worker', row)]?.error }}
                      </p>
                      <div v-else-if="(detailByKey[rowKey('worker', row)]?.items ?? []).length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-2">
                        <div
                          v-for="(prod, idx) in detailByKey[rowKey('worker', row)]?.items ?? []"
                          :key="idx"
...
/**
 * @registry-id: dailyOpsSalesByWorker
 * @last-modified: 2026-07-02T00:00:00.000Z
 * @description: Sales by worker
...
const toggleExpanded = (index: number) => {
  ...
  if (idx > -1) {
    ...
    if (row) void ensureProducts('worker', row)
  }
}
