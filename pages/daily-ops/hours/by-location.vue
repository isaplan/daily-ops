<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-3xl font-bold text-gray-900">Hours by Location</h1>
      <p class="text-gray-500">View total hours worked per location</p>
    </div>
    ...
    <UCard>
      ...
      <div class="grid gap-4 md:grid-cols-4">
        ...
        <div class="space-y-2"><label class="text-sm font-medium">Sort By</label><USelectMenu v-model="filters.sortBy" :items="[{ label: 'Location Name', value: 'location_name' }, { label: 'Total Hours', value: 'total_hours' }]" value-attribute="value" class="w-full" @update:model-value="() => fetchHours(true)" /></div>
      </div>
    ...
    <div v-if="paginationTotal > 0" class="grid gap-4 md:grid-cols-3">
      ...
      <UCard><template #header><span class="text-sm font-medium">Locations (range)</span></template><p class="text-2xl font-bold">{{ paginationTotal }}</p></UCard>
    </div>
    <UCard>
      <template #header><h2 class="font-semibold">Hours by Location</h2><p class="text-sm text-gray-500">{{ loading ? 'Loading...' : `${paginationTotal} total · ${hoursData.length} on this page` }}</p></template>
      <div v-if="loading" class="py-8 text-center text-gray-500">Loading...</div>
      <div v-else class="overflow-x-auto">
        <table class="w-full text-left text-sm">
          <thead><tr class="border-b"><th class="pb-2 pr-4 font-medium">Location</th><th class="pb-2 pr-4 font-medium">Total Hours</th><th class="pb-2 pr-4 font-medium">Total Cost</th><th class="pb-2 pr-4 font-medium">Workers</th><th class="pb-2 font-medium">Records</th></tr></thead>
          <tbody>
...
async function fetchHours (resetPage = false) {
  ...
  try {
    ...
    params.set('endpoint', filters.endpoint); params.set('groupBy', 'location'); params.set('sortBy', filters.sortBy); params.set('sortOrder', filters.sortOrder)
