<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-3xl font-bold text-gray-900">Hours by Worker</h1>
      <p class="text-gray-500">View total hours worked per worker</p>
    </div>
    ...
    <UCard>
      ...
      <div class="grid gap-4 md:grid-cols-4">
        <div class="space-y-2">
          ...
          <USelectMenu v-model="filters.source" :items="[{ label: 'Synced shifts (date range)', value: 'synced' }, { label: 'Contract totals (CSV, all-time)', value: 'contracts' }]" value-attribute="value" class="w-full" @update:model-value="fetchHours" />
        </div>
        ...
        <div class="space-y-2"><label class="text-sm font-medium">Sort By</label><USelectMenu v-model="filters.sortBy" :items="[{ label: 'Worker Name', value: 'worker_name' }, { label: 'Total Hours', value: 'total_hours' }, { label: 'Total Cost', value: 'total_cost' }]" value-attribute="value" class="w-full" @update:model-value="fetchHours" /></div>
      </div>
    ...
    <div v-if="hoursData.length > 0" class="grid gap-4 md:grid-cols-3">
      ...
      <UCard><template #header><span class="text-sm font-medium">Total Workers</span></template><p class="text-2xl font-bold">{{ hoursData.length }}</p></UCard>
    </div>
    <UCard>
      <template #header><h2 class="font-semibold">Hours by Worker</h2><p class="text-sm text-gray-500">{{ loading ? 'Loading...' : `${hoursData.length} worker(s) found` }}</p></template>
      <div v-if="loading" class="py-8 text-center text-gray-500">Loading...</div>
      <div v-else class="overflow-x-auto">
        <table class="w-full text-left text-sm">
          <thead><tr class="border-b"><th class="pb-2 pr-4 font-medium">Worker</th><th class="pb-2 pr-4 font-medium">Total Hours</th><th class="pb-2 pr-4 font-medium">Total Cost</th><th class="pb-2 pr-4 font-medium">Locations</th><th class="pb-2 font-medium">Records</th></tr></thead>
          <tbody>
            <tr v-for="(row, i) in hoursData" :key="i" class="border-b last:border-0">
              <td class="py-2 pr-4">{{ row.worker_name ?? 'Unknown' }}</td>
...
const filters = reactive({ startDate: defaultStart, endDate: defaultEnd, endpoint: 'time_registration_shifts', source: 'synced' as 'synced' | 'contracts', sortBy: 'total_hours', sortOrder: 'desc' })
...
async function fetchHours () {
  ...
  try {
    ...
    if (filters.source === 'synced') { if (filters.startDate) params.set('startDate', filters.startDate); if (filters.endDate) params.set('endDate', filters.endDate); params.set('endpoint', filters.endpoint) } else params.set('source', 'contracts')
    params.set('groupBy', 'worker'); params.set('sortBy', filters.sortBy); params.set('sortOrder', filters.sortOrder)
...
function resetFilters () { filters.startDate = defaultStart; filters.endDate = defaultEnd; filters.endpoint = 'time_registration_shifts'; filters.source = 'synced'; filters.sortBy = 'total_hours'; filters.sortOrder = 'desc'; fetchHours() }
