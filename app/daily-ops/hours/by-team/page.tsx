/**
 * @registry-id: HoursByTeamPage
 * @created: 2026-01-25T00:00:00.000Z
 * @last-modified: 2026-01-25T00:00:00.000Z
 * @description: Hours breakdown by team - shows total hours per team
 */
...
export default function HoursByTeamPage() {
  const { loading: authLoading } = useAuth();
  const [hoursData, setHoursData] = useState<HoursByTeamData[]>([]);
  ...
  const columns: DataTableColumn<HoursByTeamData>[] = [
    {
      key: 'team_name',
      header: 'Team',
      render: (row) => row.team_name || 'Unknown',
    },
    {
      key: 'total_hours',
      header: 'Total Hours',
  ...
  return (
    <div className="p-8 space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Hours by Team</h1>
        <p className="text-muted-foreground">
          View total hours worked per team
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {loading ? 'Loading...' : `${hoursData.length} ${hoursData.length === 1 ? 'team' : 'teams'} found`}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
