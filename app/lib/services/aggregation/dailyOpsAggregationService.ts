 ...
 * @description: Build Layer 3 aggregated dashboard from enriched labor and sales (all names embedded)
 * @last-fix: [2026-01-30] Initial implementation
 *
 * @exports-to:
 *   ✓ app/api/cron/daily-aggregation/route.ts
 *   ✓ app/actions/daily-ops.ts (reads v2_daily_ops_dashboard_aggregated)
 */
...
import { enrichLaborBatch } from '@/lib/services/enrichment/laborEnrichmentService';
import { enrichSalesBatch } from '@/lib/services/enrichment/salesEnrichmentService';
...
const AGGREGATED_COLLECTION = 'v2_daily_ops_dashboard_aggregated';
...
export function buildLabor(enrichedLabor: EnrichedLaborRecord[]): DashboardLabor {
...
export async function buildDailyAggregation(
  ...
  try {
    ...
    const [laborResult, salesResult] = await Promise.all([
      enrichLaborBatch(dateObj, locId),
      enrichSalesBatch(dateObj, locId),
    ]);

    const [eitjeValidation, borkValidation] = await Promise.all([
      validateEitjeTotals(date, locationId),
      validateBorkTotals(date, locationId),
    ]);

    const revenue = buildRevenue(salesResult.enriched);
    const labor = buildLabor(laborResult.enriched);
    const products = buildProducts(salesResult.enriched);
    const kpis = buildKPIs(revenue, labor);

    const sources: DashboardSources = {
      eitje: {
        hours_records: laborResult.total,
        csv_verified: eitjeValidation.all_match,
        api_verified: eitjeValidation.all_match,
      },
      bork: {
        sales_records: salesResult.total,
        total_revenue: revenue.total,
        csv_verified: borkValidation.revenue_match,
        api_verified: borkValidation.revenue_match,
      },
      validation: {
        eitje_vs_csv: { matches: eitjeValidation.all_match, variance: eitjeValidation.variance_cost },
        bork_vs_csv: { matches: borkValidation.revenue_match, variance: borkValidation.variance },
      },
    };
    ...
    await db.collection(AGGREGATED_COLLECTION).insertOne(dashboard);
    clearMasterDataCache();

    return { success: true, dashboardId: (dashboard._id as ObjectId).toString() };
  } catch (err) {
    clearMasterDataCache();
